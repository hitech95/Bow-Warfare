	...
	public static String replaceVars(String msg, HashMap<String, String>vars){
		...
		for(String s:vars.keySet()){
			try{
				...
				BowWarfare.$(Level.WARNING, "Failed to replace string vars. Error on "+s);
				error = true;
			}
		}
		if(error){
			BowWarfare.$(Level.SEVERE, "Error replacing vars in message: "+msg);
			BowWarfare.$(Level.SEVERE, "Vars: "+vars.toString());
			BowWarfare.$(Level.SEVERE, "Vars Cache: "+varcache.toString());
		}
		return msg;
	}
	...
	public static String replaceVars(String msg, String[] vars){
		...
		for(String str: varcache.keySet()){
			try{
				...
				BowWarfare.$(Level.WARNING,"Failed to replace string vars. Error on "+str);
				error = true;
			}
		}
		if(error){
			BowWarfare.$(Level.SEVERE, "Error replacing vars in message: "+msg);
			BowWarfare.$(Level.SEVERE, "Vars: "+Arrays.toString(vars));
			BowWarfare.$(Level.SEVERE, "Vars Cache: "+varcache.toString());
		}
